...
const SafetySetting = () => {
  ...
  const handleInputChange = async (e, { name, value }) => {
    ...
    if (
      ...
      name === 'Price' ||
      name === 'PayAddress' ||
      name === 'BurnCloudClientId' ||
      name === 'BurnCloudClientSecret' ||
      name === 'GitHubClientId' ||
      ...
      // 添加新的字段
      name === 'CoinbaseKey' ||
      name === 'CoinbaseWebHookKey' ||
      name === 'PaypalKey' ||
      name === 'PaypalWebHookKey'
    ) {
  ...
  const submitPayAddress = async () => {
    ...
    if (inputs.EpayKey !== undefined && inputs.EpayKey !== '') {
      ...
      // 保存 Stripe 设置
      if (originInputs['StripeKey'] !== inputs.StripeKey) {
        await updateOption('StripeKey', inputs.StripeKey);
      }
      if (originInputs['StripeWebHookKey'] !== inputs.StripeWebHookKey) {
        await updateOption('StripeWebHookKey', inputs.StripeWebHookKey);
      }

      // 保存 Coinbase 设置
      if (originInputs['CoinbaseKey'] !== inputs.CoinbaseKey) {
        await updateOption('CoinbaseKey', inputs.CoinbaseKey);
      }
      if (originInputs['CoinbaseWebHookKey'] !== inputs.CoinbaseWebHookKey) {
        await updateOption('CoinbaseWebHookKey', inputs.CoinbaseWebHookKey);
      }

      // 保存 PayPal 设置
      if (originInputs['PaypalKey'] !== inputs.PaypalKey) {
        await updateOption('PaypalKey', inputs.PaypalKey);
      }
      if (originInputs['PaypalWebHookKey'] !== inputs.PaypalWebHookKey) {
        await updateOption('PaypalWebHookKey', inputs.PaypalWebHookKey);
      }
  ...
  const submitBurnCloudOAuth = async () => {
    if (originInputs['BurnCloudClientId'] !== inputs.BurnCloudClientId) {
      await updateOption('BurnCloudClientId', inputs.BurnCloudClientId);
    }
    if (
      originInputs['BurnCloudClientSecret'] !== inputs.BurnCloudClientSecret &&
      inputs.BurnCloudClientSecret !== ''
    ) {
      await updateOption('BurnCloudClientSecret', inputs.BurnCloudClientSecret);
    }
  };
    ...
    <Grid columns={1}>
      <Grid.Column>
        <Form loading={loading} inverted={isDark}>
          ...
          <Divider />
          <Header as='h3' inverted={isDark}>
            支付设置（支持易支付、Stripe、Coinbase 和 PayPal）
          </Header>
          <Form.Group widths='equal'>
            <Form.Input
          ...
          <Form.Group widths='equal'>
            <Form.Input
              label='Coinbase API 密钥'
              placeholder='输入 Coinbase API 密钥'
              value={inputs.CoinbaseKey}
              name='CoinbaseKey'
              onChange={handleInputChange}
            />
            <Form.Input
              label='Coinbase Webhook 密钥'
              placeholder='输入 Coinbase Webhook 密钥'
              value={inputs.CoinbaseWebHookKey}
              name='CoinbaseWebHookKey'
              onChange={handleInputChange}
            />
          </Form.Group>
          ...
          <Form.Group widths='equal'>
            <Form.Input
              label='PayPal API 密钥'
              placeholder='输入 PayPal API 密钥'
              value={inputs.PaypalKey}
              name='PaypalKey'
              onChange={handleInputChange}
            />
            <Form.Input
              label='PayPal Webhook 密钥'
              placeholder='输入 PayPal Webhook 密钥'
              value={inputs.PaypalWebHookKey}
              name='PaypalWebHookKey'
              onChange={handleInputChange}
            />
          </Form.Group>
          ...
          <Form.Group inline>
            ...
            />
            <Form.Checkbox
              checked={inputs.BurnCloudOAuthEnabled === 'true'}
              label='允许通过 BurnCloud 账户登录 & 注册'
              name='BurnCloudOAuthEnabled'
              onChange={handleInputChange}
            />
            <Form.Checkbox
          ...
          <Message>
            ...
            <code>{`${inputs.ServerAddress}/oauth/burncloud`}</code>
          </Message>
          <Form.Group widths={3}>
            <Form.Input
              label='BurnCloud ID'
              ...
              onChange={handleInputChange}
              autoComplete='new-password'
              value={inputs.BurnCloudClientId}
              placeholder='输入你注册的 BurnCloud OAuth APP 的 ID'
            />
            <Form.Input
          ...
          <Form.Button onClick={submitBurnCloudOAuth}>
