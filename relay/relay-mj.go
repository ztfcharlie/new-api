...
func RelaySwapFace(c *gin.Context) *dto.MidjourneyResponse {
	...
	defer func() {
		if mjResp.StatusCode == 200 && mjResp.Response.Code == 1 {
			...
			if err != nil {
				common.SysError(lang.T(c, "mj.log.consume_error") + err.Error())
			}

			tokenName := c.GetString("token_name")
			logContent := fmt.Sprintf(lang.T(c, "mj.log.operation"), priceData.ModelPrice, priceData.GroupRatioInfo.GroupRatio, constant.MjActionSwapFace)
...
func RelayMidjourneySubmit(c *gin.Context, relayMode int) *dto.MidjourneyResponse {
	...
	defer func() {
		if consumeQuota && midjResponseWithStatus.StatusCode == 200 {
			...
			tokenName := c.GetString("token_name")
			logContent := fmt.Sprintf(lang.T(c, "mj.log.operation"), priceData.ModelPrice, priceData.GroupRatioInfo.GroupRatio, midjRequest.Action, midjResponse.Result)
			other := service.GenerateMjOtherInfo(priceData)
			model.RecordConsumeLog(c, userId, channelId, 0, 0, modelName, tokenName,
				priceData.Quota, logContent, tokenId, userQuota, 0, false, group, other)
			model.UpdateUserUsedQuotaAndRequestCount(userId, priceData.Quota)
